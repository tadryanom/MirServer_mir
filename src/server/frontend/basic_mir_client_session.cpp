...
mf::BasicMirClientSession::BasicMirClientSession(
    ...
    : session_{session},
      ...
      display_config_observer_executor{std::make_unique<ImmediateExecutor>()},
      next_id{0}
{
    output_cache.update_from(initial_display_config);
    display_config_registrar->register_interest(display_config_observer, *display_config_observer_executor);
}

mf::BasicMirClientSession::~BasicMirClientSession() = default;
